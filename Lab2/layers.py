...
class FC(Layer):
  ...
  def forward(self, inputs):
    ...
    """
    self.inputs = inputs
    ret = np.dot(inputs, self.weights.T) + self.bias
    return ret
...
class ReLU(Layer):
  ...
  def backward_inputs(self, grads):
    ...
    Returns:
      ndarray of shape (N, C, H, W).
    """
    ret = grads * (self.forward(self.inputs) > 0)
    return ret
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    # px = softmax(x)
    p_x = np.exp(x)
    p_x = p_x / np.sum(p_x, axis=1, keepdims=True)
    # cross entropy
    loss = loss - np.sum(y * np.log(p_x)) / N
    return loss
...
class L2Regularizer():
  ...
  def forward(self):
    ...
    """
    ret = self.weight_decay * np.sum(pow(self.weights, 2))
    ret = ret / 2
    return ret
