...
for i, weight_decay in enumerate(weight_decays):
    ...
    _, train_accuracy, validation_accuracy = nn.train(train_x, train_y, valid_x, valid_y, net, loss, config, dodatno=f'lambda={weight_decay}')

    # evaluate the network on the test set
    evaluation_accuracy = nn.evaluate(f"Test with weight decay factor {weight_decay}", test_x, test_y, net, loss, config)
