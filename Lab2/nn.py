...
def draw_conv_filters(epoch, step, layer, save_dir, dodatno):
  C = layer.C
  print("C", C)
  ...
  for i in range(1):
    ...
    filename = '%s_epoch_%02d_step_%s_input_%03d_%s.png' % (layer.name, epoch, step, i, dodatno)
    ski.io.imsave(os.path.join(save_dir, filename), img)


def train(train_x, train_y, valid_x, valid_y, net, loss, config, dodatno):
  ...
  for epoch in range(1, max_epochs+1):
    draw_conv_filters(0, 0, net[0], save_dir, dodatno)
    ...
    draw_conv_filters(epoch, "last", net[0], save_dir, dodatno)

    train_accuracy = (cnt_correct / num_examples * 100)
    print("Train accuracy = %.2f" % train_accuracy)
    validation_accuracy = evaluate("Validation", valid_x, valid_y, net, loss, config)
  return net, train_accuracy, validation_accuracy
...
def evaluate(name, x, y, net, loss, config):
  ...
  evaluation_accuracy = cnt_correct / num_examples * 100
  loss_avg /= num_batches
  print(name + " accuracy = %.2f" % evaluation_accuracy)
