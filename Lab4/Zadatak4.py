...
from Zadatak3b import SimpleMetricEmbedding
...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        plt.scatter(test_img_rep2d[class_indices, 0], test_img_rep2d[class_indices, 1], color=colormap[i] / 255., s=5,
                   label=i)
    #plt.scatter(test_img_rep2d[:, 0], test_img_rep2d[:, 1], color=colormap[Y[:]] / 255., s=5)
    plt.title("directly from images")
    plt.legend()
    plt.show()
    plt.figure()
    ...
    with torch.no_grad():
        ...
        for i in range(10):
            class_indices = np.where(Y == i)[0]
            plt.scatter(test_rep2d[class_indices, 0], test_rep2d[class_indices, 1], color=colormap[i] / 255.,
                        s=5,
                        label=i)
        #plt.scatter(test_rep2d[:, 0], test_rep2d[:, 1], color=colormap[Y[:]] / 255., s=5)
        plt.title("from feature representations")
        plt.legend()
        plt.show()
